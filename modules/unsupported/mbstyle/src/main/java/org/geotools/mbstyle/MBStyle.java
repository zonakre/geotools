 ...
 * <p>
 * This class is responsible for presenting the wrapped JSON in an easy to use /
 * navigate form for Java developers:
 * </p>
 * <ul>
 * <li>get methods: access the json directly</li>
 * <li>query methods: provide logic / transforms to GeoTools classes as
 * required.</li>
 * </ul>
 * <p>
 * Access methods should return Java Objects, rather than generic maps.
 * Additional access methods to perform common queries are expected and
 * encouraged.
 * </p>
 * <p>
 * This class works closely with {@link MBLayer} hierarchy used to represent the
 * fill, line, symbol, raster, circle layers. Additional support will be
 * required to work with sprties and glyphs.
 * </p>
 * 
...
public class MBStyle {
	 ...
	 * <p>
	 * All methods act as accessors on this JSON document, no other state is
	 * maintained. This allows modifications to be made cleaning with out chance
	 * of side-effect.
	 */
	JSONObject json;

	/**
	 * Helper class used to perform JSON travewrse json and perform Expression
	 * and Filter conversions.
	 */
	...
	public List<MBLayer> layers() {
		...
		Map<String, MBLayer> layersMap = new HashMap<>();
		for (Object obj : layers) {
			if (obj instanceof JSONObject) {
				JSONObject refJson = (JSONObject) obj;

				// References another layer to copy type, source, source-layer,
				// minzoom, maxzoom, filter, and layout properties from.
				MBLayer layer = null;
				if (refJson.containsKey("ref") && refJson.get("ref") instanceof String) {
					MBLayer refLayer = layersMap.get(refJson.get("ref"));
					layer = MBLayer.create(refJson, refLayer);
				} else {
					layer = MBLayer.create(refJson);
				}
	...
	public List<MBLayer> layers(String source, String sourceLayer) {
		...
		for (Object obj : layers) {
			if (obj instanceof JSONObject) {
				...
				if (source.equals(layer.getSource()) && sourceLayer.equals(layer.getSourceLayer())) {
					layersList.add(layer);
				}
			} else {
				throw new MBFormatException("Unexpected layer definition " + obj);
	...
	/**
	 * A machine-readable id for the style
	 * 
	 * @return machine-readable id, optional string.
	 */
	public String getId() {
		return parse.optional(String.class, json, "id", null);
	}
	...
	/**
	 * (Optional) Arbitrary properties useful to track with the stylesheet, but
	 * do not influence rendering. Properties should be prefixed to avoid
	...
	/**
	 * (Optional) Default map center in longitude and latitude. The style center
	 * will be used only if the map has not been positioned by other means (e.g.
	 * map options or user interaction).
	 * 
	 * @return A {@link Point} for the map center, or null if the style contains
	 *         no center.
	 */
	...
	/**
	 * (Optional) Default zoom level. The style zoom will be used only if the
	...
	/**
	 * (Optional) Default bearing, in degrees clockwise from true north. The
	 * style bearing will be used only if the map has not been positioned by
	...
	/**
	 * (Optional) Default pitch, in degrees. Zero is perpendicular to the
	 * surface, for a look straight down at the map, while a greater value like
	 * 60 looks ahead towards the horizon. The style pitch will be used only if
	 * the map has not been positioned by other means (e.g. map options or user
	 * interaction).
	 * 
	 ...
	 * .png, .json and scale factor @2x.png will be automatically appended. This
	 * property is required if any layer uses the background-pattern,
	...
	/**
	 * (Optional) A URL template for loading signed-distance-field glyph sets in
	 * PBF format. The URL must include {fontstack} and {range} tokens. This
